...
//
var process = function(sentences, numberOfTopics, numberOfTermsPerTopic, alphaValue, betaValue) {
	...
	for(var i=0;i<sentences.length;i++) {
		...
		for(var wc=0;wc<words.length;wc++) {
			...
			var wStemmed = stem(w);
			if (w=="" || !wStemmed || w.length==1 || stop_words.indexOf(w.replace("'", "")) > -1 || stop_words.indexOf(wStemmed) > -1 || w.indexOf("http")==0) continue;
			if (f[wStemmed]) { 
